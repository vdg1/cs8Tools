...
void FtpWindow::setupPaths() {
  ...
  m_defaultDir =
      ...
      "/Staubli/RobotBackups";
  ui.leDownloadDir->setText(QDir::toNativeSeparators(
      setting.value("directories/downloadDir", m_defaultDir).toString()));
  qDebug() << "Generic Data Location: "
...
void FtpWindow::setupUi() {
  ...
  connect(ui.tableViewDetectedControllers->selectionModel(),
          &QItemSelectionModel::selectionChanged, this,
          &FtpWindow::slotSelectionChanged);

  QString Octet = "(?:[0-1]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])";
  QRegExpValidator *validator =
      new QRegExpValidator(QRegExp("^" + Octet + "\\." + Octet + "\\." + Octet +
                                   "\\." + Octet + "$"),
                           this);
  ...
  ui.leIPAddress->installEventFilter(this);
  ...
  m_ScannerMovie = new QMovie(this);
  m_ScannerMovie->setFileName(":/files/resource/loading19.gif");
  ui.lbScannerStatus->setMovie(m_ScannerMovie);

  m_LoadingDataMovie = new QMovie(this);
  m_LoadingDataMovie->setFileName(":/resource/gettingDataIndicator.gif");
  connect(m_LoadingDataMovie, &QMovie::frameChanged, [=]() {
    ui.btDownload->setIcon(QIcon(m_LoadingDataMovie->currentPixmap()));
  });

  m_progressDialog = new DetailProgressDialog(this);
  connect(m_controller, &cs8Controller::transferProgress, m_progressDialog,
          &DetailProgressDialog::setDetailProgress);
...
void FtpWindow::createObjects() {
  ...
  connect(m_controller, &cs8Controller::onlineChanged, this,
          &FtpWindow::slotOnlineState);
  connect(m_controller, &cs8Controller::transferProgress, this,
          &FtpWindow::updateDataTransferRate);

  m_scanner = new cs8NetworkScanner(this);
  connect(m_scanner, &cs8NetworkScanner::scannerStarted, this,
          &FtpWindow::slotScannerStarted);
  connect(m_scanner, &cs8NetworkScanner::scannerStopped, this,
          &FtpWindow::slotScannerStopped);
  connect(m_scanner, &cs8NetworkScanner::detected, this,
          &FtpWindow::slotDetected);
  connect(m_scanner, &cs8NetworkScanner::scanCompleted, this,
          &FtpWindow::slotScanCompleted);
  /// TODO
  QTimer::singleShot(0, m_scanner, SLOT(start()));

  m_timerDelayedProfileUpdate = new QTimer(this);
  m_timerDelayedProfileUpdate->setSingleShot(true);
  m_timerDelayedProfileUpdate->setInterval(2000);
  connect(m_timerDelayedProfileUpdate, &QTimer::timeout,
          [=]() { this->updateFTPUserProfile(); });
}
...
void FtpWindow::startSyncProcess() {
  ...
  if (f.exists()) {
    ...
    if (!p.startDetached(f.fileName()))
      QMessageBox::warning(this, tr("CS8 Backup"),
                           tr("Failed to start sycronisation process"),
                           QMessageBox::Ok);
  }
...
FtpWindow::FtpWindow(QWidget *parent)
  ...
  QSettings settings;
  if (settings.value("cs8Backup/releaseNotes", "").toString() !=
...
void FtpWindow::on_btDownload_clicked() {
  if (ui.leIPAddress->findText(ui.leIPAddress->currentText()) == -1) {
    ui.leIPAddress->insertItem(ui.leIPAddress->count(),
                               ui.leIPAddress->currentText());
  }
  writeHistory();
  QSettings setting;
  setting.setValue("controllers/lastAddress",
                   ui.leIPAddress->lineEdit()->text());
  ...
  statusBar()->showMessage(
      tr("Connected to %1")
          .arg(m_serialNumber /*leIPAddress->lineEdit() ->text() ) */));
...
bool FtpWindow::storeBackupLog(const QString &serialNumber,
...
void FtpWindow::downloadFiles() {
  ...
  if (ok) {
    ...
    bool ignoreFailedDownload =
        settings.value("ignoreFailedDownloads", false).toBool();
    ...
    while (ui.fileList->count() > 0) {
      ...
      if (!isDirectory[ui.fileList->item(0)->text()]) {
        ...
        m_progressDialog->setDetailText(
            tr("Downloading %1\n to %2 %3...")
                .arg(fileName)
        ...
        if (ok) {
          QUrlInfo *infoItem = reinterpret_cast<QUrlInfo *>(
              qvariant_cast<void *>(ui.fileList->item(0)->data(Qt::UserRole)));
        ...
        if (!ok) {
          if (retry > 0) {
            ...
            if (!ignoreFailedDownload) {
              ...
              ok = true;
              completedWithoutError = false;
              protocol.append(QString("File ignored by system setting "
                                      "(ignoreFailedDownloads): %1")
                                  .arg(fileName));
            }
          }
        }
    ...
    if (completedWithoutError) {
      if (setting
              .value("profiles/" + ui.leIPAddress->lineEdit()->text() +
                         "/compress",
                     "false")
              .toBool()) {
        connect(&zip, &zipper::compressingFile, this,
                &FtpWindow::compressingFile);
        connect(&zip, SIGNAL(completed(bool)), this, (SLOT(completed(bool))));
        zip.compressDir(m_downloadDir + "/" + backupName + "/",
                        m_downloadDir + "/" + backupName + ".zip", true);
        storeBackupLog(
            m_serialNumber,
            QDir::toNativeSeparators(m_downloadDir + "/" + backupName + ".zip"),
            ui.leDownloadDir->text());
      }

      QMessageBox::information(this, tr("Backup completed"),
                               tr("Backup has been completed!"));
    } else {
...
QString FtpWindow::getSerialNumber() {
  ...
  if (controllerSerial.isEmpty() && armSerial.isEmpty()) {
    ...
    QString text = QInputDialog::getText(
        this, tr("Controller Serial Number"),
  ...
  foreach (QString path, pathList) {
    ...
    foreach (QUrlInfo info, list) {
      ...
      if (pathElements.last() != "." && pathElements.last() != "..") {
        ...
        QPixmap pixmap(!info.isFile() ? ":/images/dir.png"
                                      : ":/images/file.png");
        item->setIcon(pixmap);
...
void FtpWindow::updateDataTransferRate(qint64 /*done*/, qint64 /*total*/,
                                       qint64 rate) {
  // qDebug() << "rate" << rate << "bytes/sec";
  if (rate != 0) {
    m_transferRateLabel->setText(
        tr("Transfer Rate %1 KB/s").arg(rate / 1024, 5));
    m_progressDialog->setTotalText(
        tr("Total Progress (downloading at %1 KB/s):").arg(rate / 1024, 5));
  } else {
...
void FtpWindow::readHistory() {
  ...
  ui.leIPAddress->setEditText(
      setting.value("controllers/lastAddress", "").toString());
  ui.leIPAddress->lineEdit()->setCursorPosition(0);
}
...
bool FtpWindow::prepareDownload() {
  ...
  if (QMessageBox::question(this, tr("Confirmation"),
                            tr("Do you want to download the backup to\n%1?")
                                .arg(QDir::toNativeSeparators(m_downloadDir)),
...
void FtpWindow::saveSettings() {
  QSettings setting;
  setting.setValue("profiles/" + ui.leIPAddress->lineEdit()->text() +
                       "/username",
                   ui.leUserName->text());
  setting.setValue(
      "profiles/" + ui.leIPAddress->lineEdit()->text() + "/password",
      ui.leUserName->text().contains("staubli", Qt::CaseInsensitive)
          ? ""
          : ui.lePassword->text());
  setting.setValue("profiles/" + ui.leIPAddress->lineEdit()->text() +
                       "/backupSys",
                   ui.checkBoxSys->checkState());
  setting.setValue("profiles/" + ui.leIPAddress->lineEdit()->text() +
                       "/backupUsr",
                   ui.checkBoxUsr->checkState());
  setting.setValue("profiles/" + ui.leIPAddress->lineEdit()->text() +
                       "/backupLog",
                   ui.checkBoxLog->checkState());
  setting.setValue("profiles/" + ui.leIPAddress->lineEdit()->text() +
                       "/compress",
                   ui.checkBoxCompress->checkState());
  setting.setValue("profiles/" + ui.leIPAddress->lineEdit()->text() +
                       "/backupPartitions",
...
void FtpWindow::completed(bool result) {
  ...
  if (!result)
    QMessageBox::critical(this, tr("Error"),
                          tr("Error while compressing the backup!"),
...
void FtpWindow::slotSelectDownLoadDir() {
  QSettings settings;
  m_downloadDir =
      settings.value("directories/downloadDir", QDir::homePath() + "/robots")
          .toString();
  m_downloadDir = QFileDialog::getExistingDirectory(
      this,
...
void FtpWindow::on_action_Default_Backup_Settings_triggered() {
  ...
  QSettings setting;
  ui.leDownloadDir->setText(QDir::toNativeSeparators(
      setting.value("directories/downloadDir", QDir::homePath() + "/robots")
          .toString()));
}
...
void FtpWindow::on_btSelectDownloadDir_clicked() {
  ...
  QString newDir = QDir::toNativeSeparators(QFileDialog::getExistingDirectory(
      this, tr("Select a backup folder"), dir));
  if (!newDir.isEmpty()) {
    ui.leDownloadDir->setText(QDir::toNativeSeparators(newDir));
    if (newDir != dir) {
      QMessageBox msgBox(this);
      /*QPushButton *sessionButton = */ msgBox.addButton(
          tr("&Use the folder only for this session"), QMessageBox::AcceptRole);
      QPushButton *defaultButton = msgBox.addButton(
          tr("&Set the folder as default"), QMessageBox::RejectRole);
      msgBox.setInformativeText(
          tr("You have changed the download folder."
...
void FtpWindow::slotOnlineState(bool online, int /*error*/,
  ...
  if (!online) {
    ui.btDownload->setIcon(QIcon());
    statusBar()->showMessage(tr("Cannot connect to controller %1: %2")
                                 .arg(ui.leIPAddress->currentText())
                                 .arg(errorString));
  } else {
    ui.btDownload->setIcon(QIcon(":/resource/dialog-ok-apply.png"));
    statusBar()->showMessage(
        tr("Controller %1 (%2) is online")
            .arg(ui.leIPAddress->currentText())
...
void FtpWindow::setFileTimestamp(const QString &fileName_,
  ...
  HANDLE filename =
      CreateFile((LPCWSTR)fileName_.utf16(), FILE_WRITE_ATTRIBUTES,
                 FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING,
                 FILE_ATTRIBUTE_NORMAL, nullptr);

  // Set the file time on the file
  SetFileTime(filename, (LPFILETIME) nullptr, (LPFILETIME) nullptr,
              &thefiletime);
...
bool FtpWindow::eventFilter(QObject *obj, QEvent *event) {
  if (obj == ui.leIPAddress && event->type() == QEvent::FocusOut) {
    qDebug() << "leIpAddress lost focus";
    updateFTPUserProfile();
  }
...
void FtpWindow::slotDetected(const QString & /*host*/) {
  ui.tableViewDetectedControllers->setEnabled(true);
}

void FtpWindow::on_pushButtonExplore_clicked() {
  QDesktopServices::openUrl(
      QUrl(QString("file:///%1").arg(ui.leDownloadDir->text())));
}

void FtpWindow::updateFTPUserProfile() {
  // check if input is valid
  int cursorPos = 0;
  QString line = ui.leIPAddress->currentText();
  if (ui.leIPAddress->lineEdit()->validator()->validate(line, cursorPos) ==
      QValidator::Acceptable) {
    QSettings setting;
    ui.leUserName->setText(
        setting
            .value("profiles/" + line + "/username",
                   setting.value("defaultFTPUserName", "maintenance"))
            .toString());
    ui.lePassword->setText(
        setting
            .value("profiles/" + line + "/password",
                   setting.value("defaultPassword", "spec_cal"))
            .toString());
    ui.checkBoxSys->setChecked(
        setting
            .value("profiles/" + line + "/backupSys",
                   setting.value("defaultSys", true).toBool())
            .toBool());
    ui.checkBoxUsr->setChecked(
        setting
            .value("profiles/" + line + "/backupUsr",
                   setting.value("defaultUsr", true).toBool())
            .toBool());
    ui.checkBoxLog->setChecked(
        setting
            .value("profiles/" + line + "/backupLog",
                   setting.value("defaultLog", true).toBool())
            .toBool());
    ui.checkBoxCompress->setChecked(
        setting
            .value("profiles/" + line + "/compress",
                   setting.value("defaultCompress").toBool())
            .toBool());
    ui.checkBoxCompress->setChecked(
        setting
            .value("profiles/" + line + "/backupPartitions",
                   setting.value("backupPartitions", true).toBool())
            .toBool());
    m_controller->setAddress(ui.leIPAddress->currentText());
...
void FtpWindow::on_actionCheck_for_Updates_triggered() {
  m_checkUpdate->checkForUpdates(true, true);
}

void FtpWindow::on_leIPAddress_editTextChanged(const QString & /*arg1*/) {
  m_timerDelayedProfileUpdate->stop();
  m_timerDelayedProfileUpdate->start();
}
