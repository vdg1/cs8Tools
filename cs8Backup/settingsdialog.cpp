...
settingsDialog::settingsDialog(QWidget *parent, int page) : QDialog(parent) {
    ...
    ui.spAutomaticRetries->setValue(
        settings.value("automaticRetries", 5).toInt());
    ...
    foreach (QString language, findQmFiles()) {
        ...
        ui.cbLanguages->addItem(
            translator.translate("settingsDialog", "English"));
...
void settingsDialog::on_okButton_clicked() {
    QSettings settings;
    settings.setValue("directories/downloadDir",
                      ui.lineEditDownLoadDir->text());
    settings.setValue(
        "lang",
