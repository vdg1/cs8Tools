...
settingsDialog::settingsDialog(QWidget *parent, int page) : QDialog(parent) {
  ...
  QSettings settings;
  ui.lineEditDownLoadDir->setText(QDir::toNativeSeparators(
      settings.value("directories/downloadDir", m_defaultDir).toString()));
  ...
  ui.cbPreserveTimeStamp->setChecked(
      settings.value("preserveTimeStamp", true).toBool());
  ui.leBackupTemplate->setText(
      settings.value("backupNameTemplate", "_%SN").toString());
  ui.cbProtocol->setChecked(settings.value("saveProtocol", true).toBool());
  ui.sbRetries->setValue(settings.value("automaticRetries", 5).toInt());
  ui.cbIgnoreFaileDownloads->setChecked(
      settings.value("ignoreFailedDownloads", false).toBool());
  ui.cbUserProfile->setCurrentText(
      settings.value("defaultFTPUserName", "maintenance").toString());
  ui.lePassword->setText(
      settings.value("defaultPassword", "spec_cal").toString());
...
void settingsDialog::on_okButton_clicked() {
  ...
  settings.setValue(
      "lang",
      ui.cbLanguages->itemData(ui.cbLanguages->currentIndex(), Qt::UserRole)
          .toString());
  ...
  settings.setValue("saveProtocol", ui.cbProtocol->isChecked());
  settings.setValue("automaticRetries", ui.sbRetries->value());
  settings.setValue("ignoreFailedDownloads",
                    ui.cbIgnoreFaileDownloads->isChecked());
  settings.setValue("defaultFTPUserName", ui.cbUserProfile->currentText());
  settings.setValue("defaultPassword", ui.lePassword->text());
}
