...
logFileDetailedInformationForm::logFileDetailedInformationForm(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::logFileDetailedInformationForm)
{
    ui->setupUi(this);
    setEnabled (false);
...
logFileDetailedInformationForm::~logFileDetailedInformationForm()
{
    delete ui;
}
...
void logFileDetailedInformationForm::selectRecord(int row)
{
     m_mapper->setCurrentIndex(row);
}
